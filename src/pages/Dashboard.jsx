...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (loading) {
      console.log("Dashboard: Loading state, waiting...");
      return;
    }

    if (!isAuthenticated || !user) {
      console.log(
        "Dashboard: Not authenticated or no user, redirecting to /login"
      );
    ...
    if (user.role !== "provider") {
      console.log("Dashboard: Non-provider user, redirecting to /", {
        role: user.role,
      });
      setError("Access restricted to providers.");
      navigate("/", { replace: true });
      return;
    }

    if (!user.providerId) {
      console.error("Dashboard: No providerId for provider user", {
        userId: user.userId,
        email: user.email,
      });
      setError(
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        console.log(
          "Dashboard: Fetching data with token:",
          token?.slice(0, 10) + "..."
        );
        const [messagesRes, projectsRes, providerRes] = await Promise.all([
          fetch("http://localhost:5000/messages/provider", {
            ...
            console.error("Dashboard: Messages fetch error:", err);
          ...
          fetch(`http://localhost:5000/provider/${user.providerId}`, {
            ...
            console.error("Dashboard: Provider fetch error:", err);
        ...
        if (
          ...
          err.message.includes("Not a provider")
        ) {
          navigate("/login", { replace: true });
        }
      } finally {
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:5000/provider/${user.providerId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      const res = await fetch(
        `http://localhost:5000/provider/${user.providerId}/image`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
